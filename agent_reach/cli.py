...
def main():
    ...
    # ── diplomacy ──
    p_diplomacy = sub.add_parser("diplomacy", help="Diplomatic Impact Tracker — detect surges from Presidential trips")
    p_diplomacy_sub = p_diplomacy.add_subparsers(dest="diplomacy_command", help="Diplomacy commands")
    p_diplomacy_scan = p_diplomacy_sub.add_parser("scan", help="Run a full diplomacy scan and generate report")
    p_diplomacy_scan.add_argument("--format", choices=["text", "report"], default="text",
                                  help="Output format: text (console) or report (executive summary)")
    p_diplomacy_report = p_diplomacy_sub.add_parser("report", help="Show the latest generated report")
    p_diplomacy_report.add_argument("--format", choices=["text", "report"], default="text",
                                    help="Output format: text (console) or report (executive summary)")
    p_diplomacy_list = p_diplomacy_sub.add_parser("list", help="List all generated reports")
    p_diplomacy_keywords = p_diplomacy_sub.add_parser("keywords", help="Show tracked keyword sets")
...
def _cmd_diplomacy(args):
    """Handle diplomacy tracker commands."""
    from agent_reach.diplomat_tracker import (
        DiplomatTracker, run_diplomacy_scan, get_latest_report, keyword_sets
    )

    cmd = getattr(args, "diplomacy_command", None)

    # If no subcommand, run scan by default
    if not cmd:
        print("Running diplomacy scan (default)...")
        cmd = "scan"

    if cmd == "scan":
        fmt = getattr(args, "format", "text")
        print("🌍 Diplomatic Impact Tracker — Scanning...")
        print()
        tracker = DiplomatTracker()
        report = tracker.generate_report(force_refresh=True)
        print()
        if fmt == "report":
            print(tracker.report_to_pdf_text(report))
        else:
            print(tracker.report_to_text(report))
        print()
        print(f"Report saved: {tracker.data_dir}")
        return
    ...
    if cmd == "report":
        ...
        if not report:
            print("No reports found. Run 'agent-reach diplomacy scan' first.")
            return
        tracker = DiplomatTracker()
        if fmt == "report":
            print(tracker.report_to_pdf_text(report))
    ...
    if cmd == "list":
        ...
        if not reports:
            print("No diplomacy reports generated yet.")
            print("Run: agent-reach diplomacy scan")
            return
        print(f"📋 Diplomacy Reports ({len(reports)} total)")
        print("-" * 60)
        for r in reports:
            print(f"  {r['file']}")
            print(f"    Generated: {r['generated_at']}")
            print(f"    Alerts: {r['alert_count']}  |  Themes: {r['themes_count']}")
        print()
        print("Show a report:")
        print("  agent-reach diplomacy report")
        return

    if cmd == "keywords":
        kws = keyword_sets()
        print("📌 Tracked Diplomatic Keyword Sets")
        print("=" * 60)
        for lang, keywords in kws.items():
            print(f"\n{lang.upper()} ({len(keywords)} keywords):")
            for kw in keywords:
                print(f"  • {kw}")
        print()
        print(f"Total: {sum(len(v) for v in kws.values())} keywords across 3 languages")
        print()
        print("Customize keywords via config.yaml:")
        print("  diplomacy_keywords_russia: [\"...\", \"...\"]")
        print("  diplomacy_keywords_china:  [\"...\", \"...\"]")
        print("  diplomacy_keywords_global: [\"...\", \"...\"]")
        return
